...
function ensureSpace(ctx, doc, need) {
    if (ctx.y - need < ctx.margin + 24) {
        return newPage(doc);
    }
    return ctx;
}
function drawHairline(ctx, color = C_RULE) {
    ctx.page.drawLine({
...
function sanitizeWinAnsi(text) {
    if (!text)
        return text;
    let out = text;
    for (const [re, rep] of _WIN_ANSI_REPLACE)
...
function drawText(ctx, doc, text, opts) {
...
function drawHeading(ctx, doc, text, fonts) {
 ...
 */
function buildGraphFromVendorMeta(meta) {
    ...
    topCaps.forEach(([cat, info]) => {
        ...
        info.products.forEach((p) => {
            ;
            p.matched_topics?.forEach?.((t) => {
                if (domainIdMap.has(t))
                    matched.add(t);
            });
        });
        if (matched.size === 0) {
        ...
        matched.forEach((t) => {
            ...
            if (did)
                edges.push({ source: info.id, target: did });
        });
    });
    return { nodes, edges };
}
 ...
 */
function wrapForNode(text, font, fontSize, maxWidth) {
 ...
 */
function drawBusinessGraph(ctx, doc, graph, fonts) {
    ...
    for (const n of graph.nodes) {
        ...
        const wrapped = wrapForNode(sanitizeWinAnsi(n.label || n.id), fontForLabel, NODE_FONT_SIZE, MAX_NODE_TEXT_W);
        lineCache.set(n.id, wrapped);
        const widest = Math.max(...wrapped.map((l) => fontForLabel.widthOfTextAtSize(l, NODE_FONT_SIZE)), 40);
    ...
    for (const eIdx of g.edges()) {
        ...
        if (!eData || !eData.points || eData.points.length < 2)
            continue;
        const pts = eData.points.map((p) => ({
            x: ox + p.x * scale,
            y: topY - p.y * scale,
        }));
...
// COVER PAGE                                                     //
// ============================================================ //
function drawCoverPage(doc, c, meta, fonts) {
    const page = doc.addPage([PAGE_W, PAGE_H]);
    // Cream paper
    ...
    drawCoverFact(page, fonts, MARGIN, factsTop - 44, 'INDUSTRI', (meta.industries || []).slice(0, 3).join(' · ') || '—');
    if (typeof meta.overall_scope_score === 'number') {
        drawCoverFact(page, fonts, MARGIN + colW + 24, factsTop - 44, 'KESESUAIAN', `${Math.round((meta.overall_scope_score || 0) * 100)}%`, C_VERMILION);
    }
...
function drawCoverFact(page, fonts, x, y, label, value, valueColor = C_INK) {
    page.drawText(label, {
...
// ============================================================ //
function drawOverviewSpread(ctx, _doc, c, meta, fonts) {
    ...
    const drawSideRow = (label, value, accent = C_INK) => {
    ...
    if (typeof meta.overall_scope_score === 'number') {
        drawSideRow('Kesesuaian Domain', `${Math.round((meta.overall_scope_score || 0) * 100)}%`, C_VERMILION);
    }
    if ((meta.domain_of_interest || []).length) {
        drawSideRow('Topik DOI', (meta.domain_of_interest || []).map((s) => `#${s}`).join('  '));
    }
...
// ============================================================ //
function drawSection(ctx, doc, fonts, num, heading, body) {
    ctx = ensureSpace(ctx, doc, 100);
    if (ctx.y < 220)
        ctx = newPage(doc, fonts);
    ctx.y -= 12;
...
export async function buildVendorDossierPdf(resp, _opts = {}) {
    ...
    if (ctx.y < 340)
        ctx = newPage(doc, fonts);
    ctx = drawProsConsBlock(ctx, doc, c.pros_cons?.pros || [], c.pros_cons?.cons || [], fonts);
    // ============================================================
    // Closing
    // ============================================================
...
export async function downloadVendorDossierPdf(resp, filename) {
