...
const BY_CCA2 = new Map();
const BY_CCA3 = new Map();
...
export function resolveCountry(input) {
    ...
    if (upper.length === 2 && BY_CCA2.has(upper))
        return BY_CCA2.get(upper);
    if (upper.length === 3 && BY_CCA3.has(upper))
        return BY_CCA3.get(upper);
    const norm = normalize(raw);
    const direct = BY_NAME.get(norm);
    if (direct)
        return direct;
    const aliasCode = ALIASES[norm];
    if (aliasCode && BY_CCA2.has(aliasCode))
        return BY_CCA2.get(aliasCode);
    return null;
}
