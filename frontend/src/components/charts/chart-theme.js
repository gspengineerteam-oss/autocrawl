...
export function getPalette(isDark = currentThemeIsDark()) {
...
export const tactical = {
    ...
    get series() { return seriesPalette(); },
    get accent() { return getPalette().accent; },
    get ok() { return getPalette().ok; },
    get warn() { return getPalette().warn; },
    get crit() { return getPalette().crit; },
...
export function axisDefaults(isDark = currentThemeIsDark()) {
    ...
    return {
        ...
        axisTick: { show: false },
        splitLine: { lineStyle: { color: p.rule, type: [1, 4] } },
    };
}
/** Series palette for chart categorical color cycling. */
export function seriesPalette(isDark = currentThemeIsDark()) {
    const p = getPalette(isDark);
