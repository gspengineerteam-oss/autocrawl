...
export const useAuthStore = defineStore('auth', () => {
  ...
  const user = ref<string | null>(persisted?.user ?? null)
  const loggedInAt = ref<number | null>(persisted?.loggedInAt ?? null)
  ...
  async function login(username: string, password: string): Promise<{ ok: boolean; error?: string }> {
    ...
    try {
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ user: user.value, loggedInAt: loggedInAt.value }),
      )
    } catch {
