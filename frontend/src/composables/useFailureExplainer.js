const FAILURE_DICT = {
    no_url_no_match: {
        ...
        remedy: 'Kemungkinan nama OCR garbage dari PDF, atau company gak punya web presence. Buka detail vendor lalu klik PERDALAM SEKARANG untuk coba ulang.',
        severity: 'high',
    },
    dns_invalid: {
    ...
    completeness_low: {
        ...
        remedy: 'Turunkan VENDOR_COMPLETENESS_THRESHOLD di .env, atau klik PERDALAM SEKARANG di vendor detail.',
        severity: 'low',
    },
    scope_out_of_scope: {
        title: 'Bukan defense vendor',
        cause: 'Scope classifier reject karena hotel, news, catering, event platform, atau industry generic.',
        remedy: 'Bisa diabaikan kalau benar. KEEP_OUT_OF_SCOPE=true sudah default sehingga ref tetap ke-persist sebagai info.',
        severity: 'low',
    },
    whois_failed: {
        title: 'WHOIS lookup gagal',
        cause: 'Server registrar timeout, atau domain TLD eksotik (cn, ru, dll).',
    ...
    unknown: {
        ...
        remedy: 'Buka raw failure_reason via halaman exhibitor-refs filter unknown.',
        severity: 'medium',
    },
};
export function explainFailure(category) {
    if (!category) {
        return {
